...
export const Hero = () => {
  ...
  return (
    <section className="pt-5 pb-20 flex flex-col justify-start md:justify-center relative overflow-hidden bg-slate-950">
      ...
      <div className="max-w-5xl mx-auto px-6 w-full pt-15 md:pt-0 flex flex-col md:flex-row items-center gap-6 md:gap-16">
        ...
        {/* Lado Derecho: Logo ajustado para no empujar */}
        {/* Lado Derecho: Añadimos 'group' al contenedor padre para detectar el hover */}
        <div className="hidden md:flex flex-1 justify-center relative scale-90 lg:scale-100 group">
          <div className="absolute inset-0 bg-red-600/5 blur-[100px] rounded-full group-hover:bg-red-600/10 transition-colors duration-500"></div>
          {/* Añadimos transition-all y hover:rotate-0 */}
          <div className="relative w-64 h-64 lg:w-72 lg:h-72 bg-slate-900/40 rounded-3xl rotate-3 flex items-center justify-center border border-slate-800 shadow-2xl overflow-hidden backdrop-blur-sm transition-all duration-500 group-hover:rotate-0 group-hover:border-red-600/30 group-hover:scale-105">
            <img 
              src="/ash-dev.png" 
              alt="Logo Alexander Silva" 
              className="w-40 h-40 lg:w-44 lg:h-44 object-contain opacity-90 transition-transform duration-500 group-hover:scale-110" 
            />
          </div>
        </div>
      </div>
